...
async function boot(): Promise<void> {
  ...
  const input: FrameEauInput = {
    ...
    mgPermissiveMask: params.has('permissive') || EAU_DEFAULTS.mgPermissiveMask,
    mgAnchor: EAU_DEFAULTS.mgAnchor,
  ...
  const panel = new Panel3D(document.body, [
    {
      ...
      sliders: [
        ...
        { label: 'ancrage (ε, niveau grossier)', min: 0, max: 4, step: 0.1, get: () => input.mgAnchor, set: (x) => (input.mgAnchor = x) },
